...
const char *ssid =  "zancudo";     // Enter Network Name
const char *pass =  "aadhithya"; // Enter Network Password

#define MQTT_SERV "io.adafruit.com"
#define MQTT_PORT 1883
#define MQTT_NAME "adhithya_99"
#define MQTT_PASS "aio_EgOI174ZY2GQwnF5gMoG4Fi8N8Ow"
...
void setup() {
  ...
  timeClient.begin();      
  myservo.attach(D5);                         // Output servo to PIN D5
  ...
  Serial.print(ssid);                          // network name serial monitor output
  while (WiFi.status() != WL_CONNECTED) {
    Serial.print(".");                         // error/exceptions
    delay(500);
  }
  Serial.println();
...
void loop() {
 ...
 s3 = digitalRead(slot3);
   ...
   if (entrysensor == 1) {                     
   count=  count+1;                            // Increments value by one
   myservo.write(OPEN_ANGLE);                  //ENTRY COUNT 
   delay(3000);
   myservo.write(CLOSE_ANGLE);
   }
   
   if (exitsensor == 1) {                             
   count= count-1;                             // Decrements value by one
   myservo.write(OPEN_ANGLE);                  //EXIT COUNT
   delay(3000);                         
   myservo.write(CLOSE_ANGLE);
  }
  ...
  while ((subscription = mqtt.readSubscription(5000)))
   ...
   if (subscription == &EntryGate)
     {
      //Prints the new value to the serial monitor
...
void MQTT_connect() 
{
  ...
  uint8_t retries = 3;
  while ((ret = mqtt.connect()) != 0) // will return 0 for connected status
  { 
       mqtt.disconnect();
